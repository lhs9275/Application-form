...
for file_path in file_paths:
    for count in range(0,file_count):
        ...
        Facility = df_data.loc[count, '시설명'] if '시설명' in df_data.columns else " "
        Enter_your_home_addres = df_data.loc[count, '주소'] if '주소' in df_data.columns else " "
        Name = df_data.loc[count, '예약자'] if '예약자' in df_data.columns else " "
        Phone_Num= df_data.loc[count, '휴대폰번호'] if '휴대폰번호' in df_data.columns else " "
        BirthDay = df_data.loc[count, '사업자등록번호'] if '사업자등록번호' in df_data.columns else ""
        Answer = df_data.loc[count, '대관목적'] if '대관목적' in df_data.columns else " "
        Name_of_event = df_data.loc[count, '행사명'] if '행사명' in df_data.columns else " "
        How_many_People = df_data.loc[count, '예상 인원'] if '예상 인원' in df_data.columns else " "
        Date = df_data.loc[count, '대관일'] if '대관일' in df_data.columns else " "
        start_time = df_data.loc[count, '시작시간'] if '시작시간' in df_data.columns else " "
        end_time = df_data.loc[count, '종료시간'] if '종료시간' in df_data.columns else " "
        Money = df_data.loc[count, '총결제금액'] if '총결제금액' in df_data.columns else " "
        
        Date = Date.replace("-", "년 ", 1).replace("-", "월 ", 1) + "일"
        
        #워크시트 생성
        ws = wb.add_worksheet(f'Sheet{count+1}')
